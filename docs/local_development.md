...
```
tests/
├── conftest.py                  # Shared fixtures: db_engine (Alembic), db_session (cleanup)
...
├── integration/                 # Repository layer — real MySQL via the db container
│   └── test_booking_repository.py
└── api/                         # Full endpoint tests — TestClient + real DB
    └── test_bookings.py
```
...
│   ├── integration/    # Repository integration tests (real DB)
│   └── api/            # API endpoint tests (TestClient + real DB)
