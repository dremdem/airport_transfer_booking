...
No `.env` file is needed — `docker-compose.yml` sets all required environment variables directly.

---

## Environment Variables

`docker-compose.yml` sets the required variables directly on each service, so no `.env` file is needed to run the stack:

| Variable | Value in Compose | Description |
|----------|-----------------|-------------|
| `DATABASE_URL` | `mysql+pymysql://root:root@db:3306/transfer_bookings` | Live database (`db` resolves inside the Compose network) |
| `TEST_DATABASE_URL` | `mysql+pymysql://root:root@db:3306/transfer_bookings_test` | Isolated test database |
| `DEBUG` | `false` | Enable debug mode |

`.env.example` documents these settings for reference. Note that the hostname `db` only resolves from inside the Compose network — it is not valid if you run the app directly on the host.
...
| `make build` | Rebuild all Docker images (needed after `pyproject.toml` / `Dockerfile` / frontend changes) |
...
| `make db-clear` | Delete all booking data while keeping the schema (safe for demo resets) |
| `make db-drop` | Roll back all migrations and drop the schema (destructive — data and tables are lost) |
| `make db-connect` | Open an interactive MySQL shell in the running db container |
...
| `frontend` | local build (nginx) | `3000` | React SPA served by nginx; proxies `/api/*` to the `app` service |
...
Only selected paths are bind-mounted into the `app` container to enable hot-reload during development:
...
File changes to `app/`, `tests/`, and `alembic/` on the host are reflected immediately inside the container without a rebuild. Changes to `Dockerfile`, `pyproject.toml`, or `uv.lock` require `make build`.
...
Opens an interactive MySQL shell inside the running `db` container. Requires the `db` service to be up (`make up` or any `docker compose run` that starts it as a dependency).

Useful for inspecting rows, running ad-hoc queries, or checking schema state:
...
```
tests/
├── conftest.py                          # Shared fixtures: db engine (Alembic), db_session (cleanup)
├── unit/
│   └── test_booking_service.py          # Domain logic — no DB, no HTTP
├── integration/
│   ├── conftest.py                      # Integration client fixture (real DB, no mocks)
│   ├── test_booking_repository.py       # Repository layer against real MySQL
│   ├── test_bookings_api.py             # Multi-step lifecycle flows (full stack)
│   ├── test_create_notification_e2e.py  # BackgroundTask → notification_log wiring
│   └── test_notifications.py           # Notification side-effect tests
└── api/
    ├── conftest.py                      # API client fixture (mocked send_notification)
    └── test_bookings.py                 # HTTP contract tests — status codes, response shape
```

Unit tests have no external dependencies and run in under a second. Integration and API tests require the `db` container (started automatically via `depends_on`). The session-scoped `db_engine` fixture in `tests/conftest.py` runs `alembic upgrade head` once per pytest session and `downgrade base` on teardown, shared across all test modules.
...
├── tests/
│   ├── conftest.py     # Shared DB engine / session fixtures
│   ├── unit/           # Unit tests (no DB, no HTTP)
│   ├── integration/    # Full-stack integration tests (real DB)
│   └── api/            # HTTP contract tests (TestClient + real DB)
├── docker/
│   └── init.sql        # Creates transfer_bookings_test schema on first start
├── .env.example        # Environment variable reference (no .env file required)
